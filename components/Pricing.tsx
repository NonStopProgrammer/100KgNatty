...
const ListItem = ({ text, highlight = false }: { text: string, highlight?: boolean }) => (
  <li className="flex items-start gap-3">
    <div className={`mt-0.5 p-0.5 rounded-full ${highlight ? 'bg-lime-500 text-black shadow-[0_0_10px_rgba(132,204,22,0.5)]' : 'bg-neutral-800 text-neutral-400'}`}>
       <Check size={12} strokeWidth={4} />
    </div>
    <span className={`text-sm font-medium ${highlight ? 'text-white' : 'text-neutral-400'}`}>{text}</span>
  </li>
);
